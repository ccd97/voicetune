...
**Output:** `output/scrubbed/{call_id}_diarized.json` (same filename so translate's glob works unchanged)
...
- Batch size: 20 turns per LLM call (matches translate step)
...
- Output preserves the `_diarized.json` suffix so the translate step's glob works without modification — only the input directory changes (`output/scrubbed/` instead of `output/diarized/`)
